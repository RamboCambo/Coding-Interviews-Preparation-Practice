...
//Stefan's changes:
console.log('MONGODB_URI:', process.env.MONGODB_URI); //check if the env is exposed in the right environment
app.use(
  session({
    resave: false,
...
app.use(
  session({
    ...
    store: new MongoStore({
      url: process.env.MONGODB_URI,
      //made this change for KUBERNETES (delete if it's causing issues for docker-compose or individual docker commands (task 1+2))
      mongoOptions:{
