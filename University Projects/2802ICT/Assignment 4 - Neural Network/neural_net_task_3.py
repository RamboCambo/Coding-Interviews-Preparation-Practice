...
for batch_size in batch_sizes:
    ...
    start = time.time()
    accuracies = network.train(combined_data, combined_labels, learning_rate, epochs, batch_size)
    end = time.time()

    max_accuracy = max(accuracies)
    max_accuracies.append(max_accuracy)
    
    print(f"Batch Size: {batch_size}, Maximum Accuracy: {max_accuracy*100}%, Time Taken: {end - start}s")

plt.plot(batch_sizes, max_accuracies)
