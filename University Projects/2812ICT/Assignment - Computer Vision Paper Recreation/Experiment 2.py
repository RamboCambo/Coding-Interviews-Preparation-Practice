...
def calculate_adaptive(args):
    ...
    for i in range(gray_images.shape[0]):
        ...
        otsu_image = cv.adaptiveThreshold(img, 255, cv.ADAPTIVE_THRESH_GAUSSIAN_C, cv.THRESH_BINARY, 11, 2)
    ...
    print('Adaptive Weighted IOU Average:', np.mean(weighted_ious))
    print('Adaptive Unweighted IOU Average:', np.mean(unweighted_ious))


def calculate_otsu(args):
    gray_images = args['gray_images']
    kernel = args['kernel']
    multiplier = args['multiplier']
    rgb_images = args['rgb_images']
    ...
    print('Otsu Weighted IOU Average:', np.mean(weighted_ious))
    print('Otsu Unweighted IOU Average:', np.mean(unweighted_ious))
...
if __name__ == '__main__':
    ...
    thread = Thread(target=calculate_otsu, args=(args,))
    thread.start()
    threads.append(thread)
    
    # create a thread to calculate the IOU
    thread = Thread(target=calculate_adaptive, args=(args,))
    thread.start()
    threads.append(thread)
